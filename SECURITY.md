...
The current-user pipe DACL is not authentication against hostile processes running as the same user. Deployment checks detect accidental stale/mixed outputs; they do not establish a security boundary against an account or process compromise. GPT Gate 1 passed and production collection now uses one short-lived worker per fixed operation. Host-fatal admission/deployment/cleanup failures stop preview/export; ordinary provider/timeout/protocol operation failures remain reportable under the existing incomplete-report path.
