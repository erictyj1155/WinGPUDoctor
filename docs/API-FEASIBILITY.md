...
The [target-name contract](https://learn.microsoft.com/en-us/windows/win32/api/wingdi/ns-wingdi-displayconfig_target_device_name) permits an empty friendly name. That happened locally and is retained as `unknown/missingValue` with an explicit diagnostic; under the M3 decision recorded in [ADR 0006](decisions/0006-optional-monitor-name-severity.md), that metadata absence alone does not make the display run partial. No `DISPLAY1` heuristic or invented “Internal Display” friendly name is used. The [connector enumeration](https://learn.microsoft.com/en-us/windows/win32/api/wingdi/ne-wingdi-displayconfig_video_output_technology) supplies the observed internal-connection evidence independently.
...
## Milestone 4 worker transport — integrated

M4 adds the process and transport boundary and, after Gate 1 approval, routes the five fixed real operations through it. Production CLI composition now uses the supervisor-backed path. The implementation uses documented Windows process/containment contracts:

| API / contract | Phase 1 purpose |
|---|---|
| `CreateProcessW` with `EXTENDED_STARTUPINFO_PRESENT` | Launch the explicit framework-dependent worker host from the validated deployment directory |
| `InitializeProcThreadAttributeList`, `UpdateProcThreadAttribute`, `DeleteProcThreadAttributeList` | Attach `PROC_THREAD_ATTRIBUTE_JOB_LIST` and `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` at creation time |
| `CreateJobObjectW`, `SetInformationJobObject` | Fresh unnamed Job Object with kill-on-close, one active process, and no breakaway |
| `IsProcessInJob`, `WaitForSingleObject`, `GetExitCodeProcess`, `TerminateJobObject` | Verify containment and observe/confirm bounded process exit |
| `OpenProcessToken`, `GetTokenInformation(TokenElevation/TokenIntegrityLevel)` | Verify non-elevated parent at or below medium integrity before launch; reject an elevated child |
| `CreateNamedPipeW`, `CreateFileW` | First-instance, one-instance, byte-mode local pipe with current-user DACL, remote-client rejection, and one inheritable client handle |
| `ConvertStringSecurityDescriptorToSecurityDescriptorW` | Build the explicit current-user pipe DACL; no permissive default fallback |
| Framework `NamedPipeServerStream` over the natively opened overlapped server handle | Bounded asynchronous connect/read/write on the parent side, with absolute deadlines and retained task/cancellation/handle ownership; unconfirmed cleanup quarantines owners and poisons process-wide admission |

The worker receives exactly two inherited handles: the connected pipe client as standard input/output and write-only NUL as standard error. Job, server-pipe, process/token, console, and unrelated handles are excluded from the handle list. The parent closes client/NUL copies after attribute teardown. Raw allocation is freed on initialization failure; native attribute-list deletion occurs only after successful initialization. The current-user DACL is not authentication against hostile same-user processes. Protocol v1 is a private bounded length-prefixed UTF-8 JSON sequence (`request`, `ready`, `start`, `attemptStarted`, `result`) with fixed operation identifiers and concrete DTOs selected by explicit operation switches.

The real worker dispatcher now executes the existing narrow WMI projections and the complete existing DisplayConfig/SetupAPI topology path. The parent conditionally runs the signed-driver worker only after available nonempty video inventory, performs the exact case-insensitive driver join during snapshot assembly, and passes transient GPU labels/full WMI instance IDs only as topology input. Deployment files are hash/identity validated from the embedded worker manifest before launch. Native layout tests cover the process, token, job, startup, security-attribute, and pointer-width structures. The accepted ownership, runtime identity, protocol and production-composition contract is in [ADR 0007](decisions/0007-bounded-supervisor-and-worker-protocol.md). Its post-Gate-2 note specifies the added content-free `failure/resourceLimit` terminal variant and real attempt-boundary callbacks; neither changes Windows API contracts or structures.

Controlled cancellation uses the existing console control surface. Cancellation and normal output now compete for an atomic controller state; removing `Console.CancelKeyPress` alone cannot fence a callback already captured by the runtime. A cancellation winner stops result acceptance and uses the existing Job Object/process-exit path inside one absolute cleanup deadline. An output winner prevents a late callback from setting hidden cancellation and leaves ordinary/default termination permitted.

The opt-in live check delivers a real `CTRL_C_EVENT` with `GenerateConsoleCtrlEvent` after `AttachConsole` to the CLI's own console; the attaching helper ignores the same signal. Readiness now means parent validation of Ready, Start and the first signed-driver attempt, with the matching worker still alive. The hook captures CLI/worker PID plus birth identity before signaling and records every later worker start. Liveness checks use those identities after CLI exit, excluding console hosts and avoiding a global process-name search. The monotonic latency surrounds the successful signal call through observed CLI exit; attachment/discovery/preparation are outside it. This is end-to-end cancellation observation, not isolated cleanup duration.

Timing calibration records safe stage/operation names and durations; the separate private lifecycle probe records process metadata only into ignored evidence. Neither adds hardware identifiers or provider text to reports. The first Ready receive is after Request and does not measure the bounded connect stage. Inclusive operation timing contains provider wait, terminal frame handling and cleanup. Connect headroom and unmeasured final bookkeeping reserves are engineering judgments; [VALIDATION.md](VALIDATION.md) defines each segment and corrected rationale.

Native correction review (2026-09-20) consulted Microsoft's contracts for [attribute initialization](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-initializeprocthreadattributelist), [token information](https://learn.microsoft.com/en-us/windows/win32/api/securitybaseapi/nf-securitybaseapi-gettokeninformation), [mandatory integrity labels](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-token_mandatory_label), and [I/O cancellation](https://learn.microsoft.com/en-us/windows/win32/api/ioapiset/nf-ioapiset-cancelioex). Cancellation request does not prove completion. The fixed TOKEN_ELEVATION query uses its four-byte structure size; variable integrity data uses a bounded buffer and validates SID shape/location. Native layout checks now assert handle, attribute-list, job-limit and integrity-label offsets. Process wait conversion never emits INFINITE.
